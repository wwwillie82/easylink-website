...
const publicSection = readFileSync(new URL('../src/components/PublicSection.astro', import.meta.url), 'utf8');
...
assert.match(cardsBlock, /\{showHeader && block\.title && <h2>\{block\.title\}<\/h2>\}/);
assert.match(cardsBlock, /\{showHeader && block\.body && <p>\{block\.body\}<\/p>\}/);
assert.match(cardsBlock, /\{showHeader && block\.title && <span class="eyebrow">\{block\.title\}<\/span>\}/);
assert.match(cardsBlock, /\{showHeader && block\.body && <h2>\{block\.body\}<\/h2>\}/);
assert.match(cardsBlock, /<ListingCards items=\{cardsVm\.cards\} \/>/);
assert.match(cardsBlock, /cardsVm\.action\?\.href/);
assert.match(cardsBlock, /blockChrome/);
assert.match(cardsBlock, /showHeader/);
