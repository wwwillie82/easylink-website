...
assert.match(component, /const height = isFlow \? 600 : 680/);
assert.match(component, /const flowColumns = \[150, 370, 590, 810, 1030\]/);
assert.match(component, /const flowLanes = \[135, 300, 465\]/);
...
assert.match(component, /const flowEdgeLimit = 4/);
assert.match(component, /const flowEdgeLabelLimit = 2/);
assert.match(component, /selectVisibleFlowEdges/);
assert.match(component, /flowEdgeScore/);
assert.match(component, /score \+= 400/);
assert.match(component, /module' && toKind === 'external'/);
assert.match(component, /coreNode\?\.id \? 3 : 2/);
assert.match(component, /const svgEdges = isFlow \? selectVisibleFlowEdges\(config\?\.edges \|\| \[\], nodes, core\) : \(config\?\.edges \|\| \[\]\)/);
assert.match(component, /hasHiddenFlowEdges/);
assert.match(component, /data-network-flow-note/);
assert.match(component, /A diagram a fő kapcsolatokat emeli ki/);
assert.match(component, /svgEdges\.map/);
assert.match(component, /edgePath\(e, e\.originalIndex \?\? i\)/);
assert.match(component, /isFlow \? \(\(config\.edges\.length < 5\) && i < flowEdgeLabelLimit\)/);
assert.match(component, /isFlow \? 15 : \(config\.edges\.length >= 5 \? denseEdgeLabelMax : 22\)/);
