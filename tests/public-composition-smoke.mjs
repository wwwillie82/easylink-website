...
assert.match(contentBlocks, /return <article class:list=\{\['content-card', `type-\$\{type\}`\]\}>[\s\S]*items.length > 0/);
...
assert.match(audiencesIndex, /remainingBlocks/);
assert.match(audiencesIndex, /<ContentBlocks blocks=\{remainingBlocks\}/);
...
assert.match(integrationsIndex, /remainingBlocks/);
