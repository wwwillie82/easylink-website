...
export function createAdminRepository(pool) {
  return {
    ...
    async createPage(p) { const route = normalizeRoute(p.route || p.slug || p.title); if (route === '/') throw validationError('Adj meg érvényes URL-t.'); const [existing] = await pool.query('SELECT id FROM site_pages WHERE route=? LIMIT 1', [route]); if (existing[0]) throw validationError('Ez az URL már létezik.'); const slug = route.replace(/^\//, '').replace(/\/$/, '') || 'home'; const hero = normalizeHeroDisplay(p); const [r] = await pool.execute('INSERT INTO site_pages (route, slug, type, title, seo_title, seo_description, hero_eyebrow, hero_title, hero_description, hero_asset, hero_height, hero_image_fit, hero_image_position_x, hero_image_position_y, hero_image_position_mobile_x, hero_image_position_mobile_y, hero_overlay_strength, hero_image_scale, status, sort_order) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [route, slug, p.type || 'content_page', p.title, p.seo_title || p.title, p.seo_description || '', p.hero_eyebrow || '', p.hero_title || p.title, p.hero_description || '', p.hero_asset || '', hero.hero_height, hero.hero_image_fit, hero.hero_image_position_x, hero.hero_image_position_y, hero.hero_image_position_mobile_x, hero.hero_image_position_mobile_y, hero.hero_overlay_strength, hero.hero_image_scale, p.status || 'draft', Number(p.sort_order || 999)]); return { id: r.insertId, route, slug }; },
    ...
    async updatePage(id, p) { const current = await this.page(id); if (!current) throw new Error(`Page not found: ${id}`); const merged = { ...current.page, ...p }; const route = normalizeRoute(merged.route); const isExistingHome = current.page.route === '/' || current.page.type === 'home'; if (route === '/' && !isExistingHome) throw validationError('Adj meg érvényes URL-t.'); const [dupe] = await pool.query('SELECT id FROM site_pages WHERE route=? AND id<>? LIMIT 1', [route, id]); if (dupe[0]) throw validationError('Ez az URL már létezik.'); const slug = route === '/' ? 'home' : (merged.slug || route.replace(/^\//, '').replace(/\/$/, '') || 'home'); const hero = normalizeHeroDisplay(merged); const [r] = await pool.execute('UPDATE site_pages SET route=?, slug=?, type=?, title=?, seo_title=?, seo_description=?, hero_eyebrow=?, hero_title=?, hero_description=?, hero_asset=?, hero_height=?, hero_image_fit=?, hero_image_position_x=?, hero_image_position_y=?, hero_image_position_mobile_x=?, hero_image_position_mobile_y=?, hero_overlay_strength=?, hero_image_scale=?, status=?, sort_order=? WHERE id=?', [route, slug, merged.type, merged.title, merged.seo_title, merged.seo_description, merged.hero_eyebrow, merged.hero_title, merged.hero_description, merged.hero_asset, hero.hero_height, hero.hero_image_fit, hero.hero_image_position_x, hero.hero_image_position_y, hero.hero_image_position_mobile_x, hero.hero_image_position_mobile_y, hero.hero_overlay_strength, hero.hero_image_scale, merged.status, Number(merged.sort_order || 0), id]); if (r.affectedRows === 0) throw new Error(`Page not found: ${id}`); },
    async upsertBlock(p) {
    ...
    async createMedia(p) { const [r] = await pool.execute('INSERT INTO site_media_assets (path, alt, type, status, processing_status, staging_path, original_size_bytes, final_size_bytes, processing_error, duration_seconds, width, height) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)', [p.path, p.alt || '', p.type || '', p.status || 'active', p.processing_status || 'ready', p.staging_path || null, p.original_size_bytes || null, p.final_size_bytes || null, p.processing_error || null, p.duration_seconds || null, p.width || null, p.height || null]); return { id: r.insertId, path: p.path, alt: p.alt || '', type: p.type || '', status: p.status || 'active', processing_status: p.processing_status || 'ready', staging_path: p.staging_path || null, original_size_bytes: p.original_size_bytes || null, final_size_bytes: p.final_size_bytes || null, processing_error: p.processing_error || null, duration_seconds: p.duration_seconds || null, width: p.width || null, height: p.height || null }; },
    ...
    async claimNextMediaProcessingJob() { const conn = await pool.getConnection(); try { await conn.beginTransaction(); const [rows] = await conn.query("SELECT * FROM site_media_assets WHERE status<>'archived' AND processing_status='queued' ORDER BY id ASC LIMIT 1 FOR UPDATE SKIP LOCKED"); const media = rows[0]; if (!media) { await conn.commit(); return null; } await conn.execute("UPDATE site_media_assets SET processing_status='processing', processing_started_at=CURRENT_TIMESTAMP, processing_error=NULL WHERE id=? AND processing_status='queued'", [media.id]); await conn.commit(); return { ...media, processing_status: 'processing', processing_started_at: new Date() }; } catch (error) { await conn.rollback(); throw error; } finally { conn.release(); } },
    async recoverStaleMediaProcessingJobs({ timeoutSeconds = 3600 } = {}) { const [r] = await pool.execute("UPDATE site_media_assets SET processing_status='queued', processing_started_at=NULL, processing_error='Stale processing job recovered.' WHERE status<>'archived' AND processing_status='processing' AND processing_started_at IS NOT NULL AND processing_started_at < (CURRENT_TIMESTAMP - INTERVAL ? SECOND)", [Number(timeoutSeconds || 3600)]); return r.affectedRows || 0; },
    async markMediaReady(id, p = {}) { await pool.execute("UPDATE site_media_assets SET processing_status='ready', staging_path=NULL, original_size_bytes=?, final_size_bytes=?, processing_error=NULL, duration_seconds=?, width=?, height=?, processing_finished_at=CURRENT_TIMESTAMP WHERE id=?", [p.original_size_bytes || null, p.final_size_bytes || null, p.duration_seconds || null, p.width || null, p.height || null, id]); return this.getMedia(id); },
    async markMediaFailed(id, p = {}) { await pool.execute("UPDATE site_media_assets SET processing_status='failed', staging_path=NULL, processing_error=?, processing_finished_at=CURRENT_TIMESTAMP WHERE id=?", [p.processing_error || 'Videó feldolgozási hiba.', id]); return this.getMedia(id); },
