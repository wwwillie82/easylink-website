...
import { permissionRowsForInsert, normalizePermissions, fullAdminPermissionMatrix, adminScopes, scopeActions } from './permissions.mjs';
import { validateUserPayload, publicUser, unknownCredentialHash, rowsFromMatrix, isFullAdminMatrix, userError } from './users.mjs';
...
function stableJsonForWrite(value) { const v = jsonOrNull(value); if (v == null || v === '') return null; if (Array.isArray(v)) return v.map(stableJsonForWrite); if (typeof v === 'object') return Object.fromEntries(Object.keys(v).sort().map((key) => [key, stableJsonForWrite(v[key])])); return v; }
...
export function createAdminRepository(pool) {
  return {
    ...
    async listAdminUsers() { const [rows] = await pool.query('SELECT id,email,display_name,status,created_at,updated_at,last_login_at FROM site_admin_users ORDER BY display_name,email'); return rows; },
    async getAdminUserWithPermissions(id) { const [rows] = await pool.query('SELECT id,email,display_name,status,created_at,updated_at,last_login_at FROM site_admin_users WHERE id=? LIMIT 1', [id]); if (!rows[0]) return null; return publicUser(rows[0], await this.loadAdminUserScopes(id)); },
    async createAdminUserWithPermissions(payload) { const data = validateUserPayload(payload, { create: true }); const conn = await pool.getConnection(); try { await conn.beginTransaction(); const [dupe] = await conn.query('SELECT id FROM site_admin_users WHERE email=? LIMIT 1 FOR UPDATE', [data.email]); if (dupe[0]) throw userError(409, 'DUPLICATE_EMAIL', 'Ez az e-mail-cím már használatban van.'); const [r] = await conn.execute("INSERT INTO site_admin_users (email,password_hash,display_name,role,status) VALUES (?,?,?,'admin','active')", [data.email, unknownCredentialHash(), data.display_name]); for (const row of rowsFromMatrix(data.permissions)) await conn.execute('INSERT INTO site_admin_user_scopes (admin_user_id,scope_code,can_save,can_archive,can_delete,can_republish,can_restore) VALUES (?,?,?,?,?,?,?)', [r.insertId,row.scope_code,row.can_save,row.can_archive,row.can_delete,row.can_republish,row.can_restore]); await conn.commit(); return await this.getAdminUserWithPermissions(r.insertId); } catch (error) { await conn.rollback(); if (error.code === 'ER_DUP_ENTRY') throw userError(409, 'DUPLICATE_EMAIL', 'Ez az e-mail-cím már használatban van.'); throw error; } finally { conn.release(); } },
    async updateAdminUserWithPermissions(id, payload, actorId) { const data = validateUserPayload(payload); const conn = await pool.getConnection(); try { await conn.beginTransaction(); const [rows] = await conn.query('SELECT id,email,display_name,status FROM site_admin_users WHERE id=? LIMIT 1 FOR UPDATE', [id]); const current = rows[0]; if (!current) throw userError(404, 'USER_NOT_FOUND', 'A felhasználó nem található.'); if (Number(actorId) === Number(id) && current.status === 'active' && data.status === 'disabled') throw userError(409, 'SELF_DISABLE_FORBIDDEN', 'A saját felhasználó nem tiltható le.'); const [dupe] = await conn.query('SELECT id FROM site_admin_users WHERE email=? AND id<>? LIMIT 1 FOR UPDATE', [data.email, id]); if (dupe[0]) throw userError(409, 'DUPLICATE_EMAIL', 'Ez az e-mail-cím már használatban van.'); const beforePerms = await this.loadAdminUserScopes(id); await conn.execute('UPDATE site_admin_users SET email=?,display_name=?,status=? WHERE id=?', [data.email,data.display_name,data.status,id]); await conn.execute('DELETE FROM site_admin_user_scopes WHERE admin_user_id=?', [id]); for (const row of rowsFromMatrix(data.permissions)) await conn.execute('INSERT INTO site_admin_user_scopes (admin_user_id,scope_code,can_save,can_archive,can_delete,can_republish,can_restore) VALUES (?,?,?,?,?,?,?)', [id,row.scope_code,row.can_save,row.can_archive,row.can_delete,row.can_republish,row.can_restore]); if (data.status === 'disabled') await conn.execute('UPDATE site_admin_sessions SET revoked_at=CURRENT_TIMESTAMP WHERE admin_user_id=? AND revoked_at IS NULL', [id]); const [all] = await conn.query('SELECT id,status FROM site_admin_users WHERE status=? ORDER BY id FOR UPDATE', ['active']); let fullCount = 0; for (const u of all) { const [pr] = await conn.query('SELECT * FROM site_admin_user_scopes WHERE admin_user_id=?', [u.id]); if (isFullAdminMatrix(normalizePermissions(pr))) fullCount++; } if (fullCount < 1) throw userError(409, 'LAST_FULL_ADMIN_REQUIRED', 'Legalább egy teljes jogosultságú aktív adminnak maradnia kell.'); await conn.commit(); return await this.getAdminUserWithPermissions(id); } catch (error) { await conn.rollback(); if (error.code === 'ER_DUP_ENTRY') throw userError(409, 'DUPLICATE_EMAIL', 'Ez az e-mail-cím már használatban van.'); throw error; } finally { conn.release(); } },
    async revokeAdminUserSessions(id) { await pool.execute('UPDATE site_admin_sessions SET revoked_at=CURRENT_TIMESTAMP WHERE admin_user_id=? AND revoked_at IS NULL', [id]); },
    async createAdminPasswordResetToken(adminUserId, hash, { requestedIp = null } = {}) { const conn = await pool.getConnection(); try { await conn.beginTransaction(); const [recent] = await conn.query('SELECT id FROM site_admin_password_reset_tokens WHERE admin_user_id=? AND used_at IS NULL AND created_at > CURRENT_TIMESTAMP - INTERVAL 5 MINUTE LIMIT 1 FOR UPDATE', [adminUserId]); if (recent[0]) throw userError(429, 'PASSWORD_RESET_THROTTLED', 'Ehhez a felhasználóhoz 5 percenként küldhető új link.'); await conn.execute('UPDATE site_admin_password_reset_tokens SET used_at=CURRENT_TIMESTAMP WHERE admin_user_id=? AND used_at IS NULL', [adminUserId]); await conn.execute('INSERT INTO site_admin_password_reset_tokens (admin_user_id,token_hash,expires_at,requested_ip) VALUES (?,?,CURRENT_TIMESTAMP + INTERVAL 60 MINUTE,?)', [adminUserId, hash, requestedIp]); await conn.commit(); } catch (e) { await conn.rollback(); throw e; } finally { conn.release(); } },
    async consumeAdminPasswordResetToken(hash, passwordHash) { const conn = await pool.getConnection(); try { await conn.beginTransaction(); const [rows] = await conn.query('SELECT t.*,u.status FROM site_admin_password_reset_tokens t JOIN site_admin_users u ON u.id=t.admin_user_id WHERE t.token_hash=? LIMIT 1 FOR UPDATE', [hash]); const t = rows[0]; if (!t || t.used_at || new Date(t.expires_at).getTime() < Date.now() || t.status !== 'active') throw userError(400, 'INVALID_RESET_TOKEN', 'A jelszóbeállító link érvénytelen vagy lejárt.'); await conn.execute('UPDATE site_admin_users SET password_hash=? WHERE id=?', [passwordHash, t.admin_user_id]); await conn.execute('UPDATE site_admin_password_reset_tokens SET used_at=CURRENT_TIMESTAMP WHERE admin_user_id=? AND used_at IS NULL', [t.admin_user_id]); await conn.execute('UPDATE site_admin_sessions SET revoked_at=CURRENT_TIMESTAMP WHERE admin_user_id=? AND revoked_at IS NULL', [t.admin_user_id]); await conn.commit(); } catch(e){ await conn.rollback(); throw e; } finally { conn.release(); } },
    ...
    async findActiveAdminUserById(id) { const [r] = await pool.query("SELECT id,email,display_name,status FROM site_admin_users WHERE id=? AND status='active' LIMIT 1", [id]); return r[0] || null; },
    async markAdminLogin(id) { await pool.execute('UPDATE site_admin_users SET last_login_at=CURRENT_TIMESTAMP WHERE id=?', [id]); },
    async loadAdminUserScopes(id) { const [r] = await pool.query('SELECT * FROM site_admin_user_scopes WHERE admin_user_id=? ORDER BY scope_code', [id]); return r; },
    async ensureAdminPermissionMatrix(id, matrix) { for (const row of permissionRowsForInsert(matrix)) await pool.execute('INSERT IGNORE INTO site_admin_user_scopes (admin_user_id,scope_code,can_save,can_archive,can_delete,can_republish,can_restore) VALUES (?,?,?,?,?,?,?)', [id, row.scope_code, row.can_save, row.can_archive, row.can_delete, row.can_republish, row.can_restore]); },
    async replaceAdminPermissionMatrix(id, matrix) { const conn = await pool.getConnection(); try { await conn.beginTransaction(); await conn.execute('DELETE FROM site_admin_user_scopes WHERE admin_user_id=?', [id]); for (const row of permissionRowsForInsert(matrix)) await conn.execute('INSERT INTO site_admin_user_scopes (admin_user_id,scope_code,can_save,can_archive,can_delete,can_republish,can_restore) VALUES (?,?,?,?,?,?,?)', [id, row.scope_code, row.can_save, row.can_archive, row.can_delete, row.can_republish, row.can_restore]); await conn.commit(); } catch (error) { await conn.rollback(); throw error; } finally { conn.release(); } },
    async createAdminSession(p) { const [r] = await pool.execute('INSERT INTO site_admin_sessions (admin_user_id,token_hash,csrf_token_hash,expires_at) VALUES (?,?,?,?)', [p.admin_user_id, p.token_hash, p.csrf_token_hash, p.expires_at]); return { id: r.insertId, ...p }; },
    async resolveAdminSessionByTokenHash(hash) { const [r] = await pool.query('SELECT s.*, u.id AS user_id, u.email, u.display_name, u.status FROM site_admin_sessions s JOIN site_admin_users u ON u.id=s.admin_user_id WHERE s.token_hash=? LIMIT 1', [hash]); const row = r[0]; if (!row) return null; return { session: { id: row.id, admin_user_id: row.admin_user_id, expires_at: row.expires_at, revoked_at: row.revoked_at }, user: { id: row.user_id, email: row.email, display_name: row.display_name, status: row.status } }; },
    async getAdminSessionCsrfHash(id) { const [r] = await pool.query('SELECT csrf_token_hash FROM site_admin_sessions WHERE id=? LIMIT 1', [id]); return r[0]?.csrf_token_hash || null; },
    async revokeAdminSession(id) { await pool.execute('UPDATE site_admin_sessions SET revoked_at=CURRENT_TIMESTAMP WHERE id=? AND revoked_at IS NULL', [id]); },
    async revokeAllAdminSessions(id) { await pool.execute('UPDATE site_admin_sessions SET revoked_at=CURRENT_TIMESTAMP WHERE admin_user_id=? AND revoked_at IS NULL', [id]); },
    async touchAdminSession(id) { await pool.execute('UPDATE site_admin_sessions SET last_seen_at=CURRENT_TIMESTAMP WHERE id=? AND (last_seen_at IS NULL OR last_seen_at < CURRENT_TIMESTAMP - INTERVAL 5 MINUTE)', [id]); },
    ...
    async block(id) { const [r] = await pool.query('SELECT * FROM site_content_blocks WHERE id=? LIMIT 1', [id]); return r[0] || null; },
...
if (Array.isArray(payload.blocks)) {
          ...
          for (const [index, raw] of payload.blocks.entries()) {
            ...
            planned.push({ existing, blockId, client_key: raw.client_key || '', block_key, type, title: raw.title || '', body: raw.body || '', items, presentation: raw.presentation ?? existing?.presentation ?? null, sort_order, status: raw.status || 'published' });
          }
          ...
          for (const item of planned) {
            if (item.existing) { const [r] = await conn.execute('UPDATE site_content_blocks SET type=?, title=?, body=?, items=?, presentation=?, sort_order=?, status=? WHERE id=? AND page_id=?', [item.type, item.title, item.body, JSON.stringify(item.items), item.presentation ? JSON.stringify(stableJsonForWrite(item.presentation)) : null, item.sort_order, item.status, item.blockId, id]); if (r.affectedRows !== 1) throw new Error(`Block update failed: ${item.blockId}`); }
            else { const [r] = await conn.execute('INSERT INTO site_content_blocks (page_id, block_key, type, title, body, items, presentation, sort_order, status) VALUES (?,?,?,?,?,?,?,?,?)', [id, item.block_key, item.type, item.title, item.body, JSON.stringify(item.items), item.presentation ? JSON.stringify(stableJsonForWrite(item.presentation)) : null, item.sort_order, item.status]); item.blockId = r.insertId; blockMappings.push({ client_key: item.client_key, id: r.insertId, block_key: item.block_key }); }
          }
