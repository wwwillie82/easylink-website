...
export function createAdminRepository(pool) {
  return {
    ...
    async upsertBlock(p) {
      ...
      let targetPages = [];
      if (['cards','card-grid'].includes(String(p.type))) { const ids = [...new Set((Array.isArray(rawItems) ? rawItems : []).flatMap((entry) => entry?.version === 2 ? [...(entry.cards || []), entry.action || null] : [entry]).map((entry) => Number(entry?.target_page_id)).filter((id) => Number.isSafeInteger(id) && id > 0))]; if (ids.length) { const [rows] = await pool.query(`SELECT id,route,slug,type,title,status,seo_description,hero_description,sort_order FROM site_pages WHERE id IN (${ids.map(()=>'?').join(',')}) ORDER BY id`, ids); targetPages = rows; } }
      const items = ['cards','card-grid'].includes(String(p.type)) ? normalizeBlockItemsByType(p.type, rawItems, { pages: targetPages, requirePublishedTargets: true, path: 'items' }) : normalizeBlockItems(p.type, rawItems);
