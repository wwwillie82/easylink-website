...
import { auditPanel } from './render/audit.mjs';
...
import { auditChangedFields, auditRequestContext, writeAuditEvent } from './audit.mjs';
...
export function createAdminServer({ repo, env = process.env, publishService } = {}) {
  ...
  async function audit(req, event) { try { await writeAuditEvent(repo, req, event); } catch (error) { console.error('admin audit insert failed', { code: error.code, message: error.message }); } }
  async function auditedPublish(req, user, label, rollbackSourceId = null) {
    const started = Date.now();
    const result = await publishAfterSave(user, label);
    const ok = result?.ok !== false && !result?.status?.includes?.('failed');
    await audit(req, { event_code: rollbackSourceId ? (ok ? 'admin_publish_rollback_completed' : 'admin_publish_rollback_failed') : (ok ? 'admin_publish_completed' : 'admin_publish_failed'), result: ok ? 'success' : 'failure', actor: user, scope_code: 'publish', action_code: rollbackSourceId ? 'restore' : 'republish', target_type: 'publish_snapshot', target_id: result?.snapshotId || rollbackSourceId || null, target_label: rollbackSourceId ? `Rollback: ${rollbackSourceId}` : label, metadata: { snapshotId: result?.snapshotId, sourceSnapshotId: rollbackSourceId, status: result?.status, durationMs: Date.now() - started, errorCode: result?.error ? 'PUBLISH_ERROR' : null } });
    return result;
  }
  ...
  return http.createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === '/api/admin/password-reset/request' && req.method === 'POST') { if (!sameOriginOk(req)) return apiError(res, 403, 'FORBIDDEN', 'Érvénytelen kérés.'); const resetPayload = await body(req); const result = await requestPasswordReset(repo, resetPayload.email, { env, requestedIp: req.socket?.remoteAddress || null }); await audit(req, { event_code: 'password_reset_requested', result: 'success', target_type: 'admin_login', target_label: resetPayload.email, metadata: { publicRequest: true } }); return json(res, 200, { ok: true, message: result.message }); }
      if (url.pathname === '/api/admin/password-reset/confirm' && req.method === 'POST') { if (!sameOriginOk(req)) return apiError(res, 403, 'FORBIDDEN', 'Érvénytelen kérés.'); try { const result = await confirmPasswordReset(repo, await body(req)); await audit(req, { event_code: 'password_reset_completed', result: 'success', target_type: 'admin_login' }); await audit(req, { event_code: 'admin_session_revoked', result: 'success', target_type: 'admin_login', metadata: { reason: 'password_reset' } }); return json(res, 200, { ok: true, data: result }); } catch (error) { await audit(req, { event_code: 'password_reset_failed', result: 'failure', target_type: 'admin_login', metadata: { errorCode: error.code || 'INVALID_RESET_TOKEN' } }); return apiError(res, error.status || 400, error.code || 'INVALID_RESET_TOKEN', error.message || 'A jelszóbeállító link érvénytelen vagy lejárt.'); } }
      ...
      if (url.pathname === '/api/admin/login' && req.method === 'POST') {
        ...
        if (!valid.ok) { await audit(req, { event_code: 'admin_login_failed', result: 'failure', target_type: 'admin_login', target_label: payload?.email, metadata: { reason: 'invalid_payload' } }); return wantsHtml(req) ? (res.writeHead(400, { 'content-type': 'text/html; charset=utf-8' }), res.end(loginHtml(valid.error.message))) : json(res, 400, valid); }
        if (!sameOriginOk(req)) { await audit(req, { event_code: 'admin_csrf_rejected', result: 'denied', target_type: 'admin_login', target_label: valid.data.email, metadata: { method: req.method, pathname: url.pathname } }); return apiError(res, 403, 'FORBIDDEN', 'Érvénytelen belépési kérés.'); }
        const user = await authenticate(repo, valid.data.email, valid.data.password);
        if (!user) { await audit(req, { event_code: 'admin_login_failed', result: 'failure', target_type: 'admin_login', target_label: valid.data.email, metadata: { reason: 'invalid_credentials' } }); return wantsHtml(req) ? (res.writeHead(401, { 'content-type': 'text/html; charset=utf-8' }), res.end(loginHtml('Hibás belépési adatok.'))) : apiError(res, 401, 'INVALID_CREDENTIALS', 'Hibás belépési adatok.'); }
        const session = await createLoginSession(repo, user, env);
        await audit(req, { event_code: 'admin_login_succeeded', result: 'success', actor: user, target_type: 'admin_user', target_id: user.id, target_label: user.displayName || user.email });
      ...
      if (url.pathname === '/api/admin/logout' && req.method === 'POST') { await repo.revokeAdminSession?.(adminContext.session.id); await audit(req, { event_code: 'admin_logout', result: 'success', actor: user, target_type: 'admin_user', target_id: user.id, target_label: user.displayName || user.email }); return redirect(res, '/admin/login', { 'set-cookie': clearAuthCookies(env) }); }
      if (url.pathname === '/api/admin/audit' && req.method === 'GET') { const result = await repo.listAuditEvents(Object.fromEntries(url.searchParams)); return json(res, 200, { ok: true, data: result.data, pagination: result.pagination }); }
      ...
      if (url.pathname === '/api/admin/users') {
        ...
        try { const payload = await body(req); const data = await repo.createAdminUserWithPermissions(payload); await audit(req, { event_code: 'admin_user_created', result: 'success', actor: user, scope_code: 'users', action_code: 'save', target_type: 'admin_user', target_id: data.id, target_label: data.display_name || data.email, metadata: { scopes: Object.keys(data.permissions || {}) } }); let reset = { ok: true }; try { await issuePasswordReset(repo, data, { env, requestedIp: req.socket?.remoteAddress || null }); await audit(req, { event_code: 'admin_user_reset_link_requested', result: 'success', actor: user, scope_code: 'users', action_code: 'save', target_type: 'admin_user', target_id: data.id, target_label: data.display_name || data.email }); } catch (error) { reset = { ok: false, code: error.code || 'SEND_FAILED', message: error.message || 'A link küldése sikertelen, küldd újra később.' }; await audit(req, { event_code: 'admin_user_reset_link_requested', result: 'failure', actor: user, scope_code: 'users', action_code: 'save', target_type: 'admin_user', target_id: data.id, target_label: data.display_name || data.email, metadata: { errorCode: reset.code } }); } return json(res, 200, { ok: true, data, reset }); } catch (error) { await audit(req, { event_code: 'admin_user_created', result: error.code?.includes('FORBIDDEN') ? 'denied' : 'failure', actor: user, scope_code: 'users', action_code: 'save', metadata: { errorCode: error.code || 'INVALID_USER' } }); return apiError(res, error.status || 400, error.code || 'INVALID_USER', error.message); }
      }
      if (/^\/api\/admin\/users\/\d+(?:\/(?:revoke-sessions|send-reset-link))?$/.test(url.pathname)) {
        ...
        if (url.pathname.endsWith('/revoke-sessions')) { if (req.method !== 'POST') return apiError(res, 405, 'METHOD_NOT_ALLOWED', 'Nem támogatott HTTP metódus.'); await repo.revokeAdminUserSessions(id); await audit(req, { event_code: 'admin_user_sessions_revoked', result: 'success', actor: user, scope_code: 'users', action_code: 'archive', target_type: 'admin_user', target_id: id }); await audit(req, { event_code: 'admin_session_revoked', result: 'success', actor: user, target_type: 'admin_user', target_id: id }); return json(res, 200, { ok: true, data: { selfRevoked: Number(id) === Number(user.id) } }); }
        if (url.pathname.endsWith('/send-reset-link')) { if (req.method !== 'POST') return apiError(res, 405, 'METHOD_NOT_ALLOWED', 'Nem támogatott HTTP metódus.'); const target = await repo.getAdminUserWithPermissions(id); if (!target || target.status !== 'active') return apiError(res, 404, 'USER_NOT_FOUND', 'Aktív felhasználó nem található.'); try { await issuePasswordReset(repo, target, { env, requestedIp: req.socket?.remoteAddress || null }); await audit(req, { event_code: 'admin_user_reset_link_requested', result: 'success', actor: user, scope_code: 'users', action_code: 'save', target_type: 'admin_user', target_id: id, target_label: target.display_name || target.email }); return json(res, 200, { ok: true }); } catch (error) { return apiError(res, error.status || 503, error.code || 'SEND_FAILED', error.message || 'A küldés sikertelen.'); } }
        ...
        try { const updated = await repo.updateAdminUserWithPermissions(id, payload, user.id); const ev = current.status === 'disabled' && updated.status === 'active' ? 'admin_user_reactivated' : (current.status === 'active' && updated.status === 'disabled' ? 'admin_user_disabled' : 'admin_user_updated'); await audit(req, { event_code: ev, result: 'success', actor: user, scope_code: 'users', action_code: ev === 'admin_user_disabled' ? 'archive' : 'save', target_type: 'admin_user', target_id: id, target_label: updated.display_name || updated.email, metadata: { changedFields: auditChangedFields(current, updated, ['email','display_name','status']) } }); if (JSON.stringify(current.permissions||{}) !== JSON.stringify(updated.permissions||{})) await audit(req, { event_code: 'admin_user_permissions_changed', result: 'success', actor: user, scope_code: 'users', action_code: 'save', target_type: 'admin_user', target_id: id, target_label: updated.display_name || updated.email, metadata: { before: current.permissions, after: updated.permissions } }); return json(res, 200, { ok: true, data: updated }); } catch (error) { return apiError(res, error.status || 400, error.code || 'INVALID_USER', error.message); }
      }
      ...
      if (url.pathname.startsWith('/api/admin/pages/')) {
        ...
        try { const payload = await body(req); const cls = buildPageEffectiveMutationPlan(page.page || page, payload); if (cls.noOp) return apiError(res, 400, 'INVALID_EMPTY_MUTATION', 'Nincs menthető változás.'); if ((cls.needsSave && !hasAction(adminContext.permissions, 'pages', 'save')) || (cls.needsArchive && !hasAction(adminContext.permissions, 'pages', 'archive'))) return apiError(res, 403, 'FORBIDDEN', 'Nincs jogosultság a művelethez.'); await repo.updatePage(id, cls.next || payload); const ev = page.page?.status !== 'archived' && (cls.next || payload).status === 'archived' ? 'admin_page_archived' : (page.page?.status === 'archived' && (cls.next || payload).status && (cls.next || payload).status !== 'archived' ? 'admin_page_reactivated' : 'admin_page_updated'); await audit(req, { event_code: ev, result: 'success', actor: user, scope_code: 'pages', action_code: ev === 'admin_page_archived' ? 'archive' : 'save', target_type: 'page', target_id: id, target_label: page.page?.title || page.page?.route, metadata: { changedFields: auditChangedFields(page.page || page, cls.next || payload, ['title','route','status','type','slug','sort_order']), previousStatus: page.page?.status, nextStatus: (cls.next || payload).status, previousRoute: page.page?.route, nextRoute: (cls.next || payload).route } }); } catch (error) { if (error.code === 'HOME_USE_HOME_EDITOR') return apiError(res, 409, 'HOME_USE_HOME_EDITOR', error.message); if (error.code === 'HOME_CANONICAL_USE_HOME_EDITOR') return apiError(res, 409, 'HOME_CANONICAL_USE_HOME_EDITOR', error.message); if (error.code === 'PAGE_IN_USE') return apiError(res, 409, 'PAGE_IN_USE', error.message, error.details); if (error.code === 'DUPLICATE_NAVIGATION_HREF') return apiError(res, 409, 'DUPLICATE_NAVIGATION_HREF', error.message, error.details); if (error.status === 400 || error.code === 'VALIDATION_ERROR') return apiError(res, 400, 'INVALID_PAGE', error.message); throw error; }
        return json(res, 200, { ok: true, publish: await auditedPublish(req, user, `Oldal mentés: ${id}`) });
      }
      if (url.pathname === '/api/admin/blocks' && req.method === 'POST') {
        ...
        await audit(req, { event_code: payload.id ? (currentBlock?.status === 'archived' && (cls.next||payload).status !== 'archived' ? 'admin_block_reactivated' : ((cls.next||payload).status === 'archived' ? 'admin_block_archived' : 'admin_block_updated')) : 'admin_block_created', result: 'success', actor: user, scope_code: 'pages', action_code: ((cls.next||payload).status === 'archived') ? 'archive' : 'save', target_type: 'block', target_id: data.id || payload.id, target_label: payload.title || payload.type, metadata: { pageId: payload.page_id, blockType: payload.type, changedFields: auditChangedFields(currentBlock || {}, cls.next || payload, ['type','title','status','sort_order']) } }); return json(res, 200, { ok: true, data, publish: await auditedPublish(req, user, `Blokk mentés: ${payload.page_id}`) });
      }
      if (url.pathname.startsWith('/api/admin/blocks/') && req.method === 'DELETE') { try { const blockId = url.pathname.split('/').pop(); const oldBlock = await repo.block?.(blockId); await repo.deleteBlock(blockId); await audit(req, { event_code: 'admin_block_deleted', result: 'success', actor: user, scope_code: 'pages', action_code: 'archive', target_type: 'block', target_id: blockId, target_label: oldBlock?.title || oldBlock?.type, metadata: { pageId: oldBlock?.page_id, blockType: oldBlock?.type, previousStatus: oldBlock?.status, nextStatus: 'archived' } }); return json(res, 200, { ok: true, publish: await auditedPublish(req, user, `Blokk inaktiválás`) }); } catch (error) { if (error.code === 'HOME_CANONICAL_USE_HOME_EDITOR') return apiError(res, 409, 'HOME_CANONICAL_USE_HOME_EDITOR', error.message); throw error; } }
      if (url.pathname === '/api/admin/settings') { if (!['GET','PUT','POST'].includes(req.method)) return apiError(res, 405, 'METHOD_NOT_ALLOWED', 'Nem támogatott HTTP metódus.'); if (req.method === 'GET') return json(res, 200, { ok: true, data: await repo.getSiteSettings() }); if (req.method === 'PUT' || req.method === 'POST') { try { const settingsPayload = await body(req); const data = await repo.updateSiteSettings(settingsPayload, env); await audit(req, { event_code: 'admin_settings_updated', result: 'success', actor: user, scope_code: 'settings', action_code: 'save', target_type: 'settings', target_label: 'Alapadatok', metadata: { changedKeys: Object.keys(settingsPayload || {}) } }); return json(res, 200, { ok: true, data, publish: await auditedPublish(req, user, 'Alapadatok mentés') }); } catch (error) { if (error.status === 400 || error.code === 'VALIDATION_ERROR') return apiError(res, 400, 'INVALID_SETTINGS', error.message); throw error; } } }
      if (url.pathname === '/api/admin/navigation') { if (!['GET','POST','PUT'].includes(req.method)) return apiError(res, 405, 'METHOD_NOT_ALLOWED', 'Nem támogatott HTTP metódus.'); if (req.method === 'GET') return json(res, 200, { ok: true, data: await repo.nav() }); const pages = await repo.pages(); const navPayload = await body(req); const currentNav = await repo.nav(); const navCls = buildNavigationEffectiveMutationPlan(currentNav, navPayload.items || []); if (navCls.noOp) return apiError(res, 400, 'INVALID_EMPTY_MUTATION', 'Nincs menthető változás.'); if ((navCls.needsSave && !hasAction(adminContext.permissions, 'menu', 'save')) || (navCls.needsArchive && !hasAction(adminContext.permissions, 'menu', 'archive'))) return apiError(res, 403, 'FORBIDDEN', 'Nincs jogosultság a művelethez.'); const valid = validateNavPayload(navPayload, pages); if (!valid.ok) return json(res, valid.error?.code === 'NAVIGATION_TARGET_PAGE_NOT_PUBLISHED' ? 409 : 400, valid); let navigationIds; try { navigationIds = await repo.updateNav(navCls.nextRows || valid.data); } catch (error) { if (error.code === 'NAVIGATION_TARGET_PAGE_NOT_PUBLISHED') return apiError(res, 409, 'NAVIGATION_TARGET_PAGE_NOT_PUBLISHED', error.message, error.details); if (error.status === 400 || error.code === 'VALIDATION_ERROR' || error.code === 'ER_DUP_ENTRY') return apiError(res, 400, navValidationCode(error), error.code === 'ER_DUP_ENTRY' || /Duplikált menüpont link/i.test(String(error.message || '')) ? 'Duplikált menüpont link.' : error.message); throw error; } await audit(req, { event_code: 'admin_navigation_saved', result: 'success', actor: user, scope_code: 'menu', action_code: 'save', target_type: 'navigation', target_label: 'Menü mentés', metadata: { itemCount: valid.data.length, navigationIds: Array.from(navigationIds || []) } }); return json(res, 200, { ok: true, data: { navigationIds: Array.from(navigationIds || []), navigationMappings: navigationIds?.navigationMappings || navigationIds?.clientMappings || [] }, publish: await auditedPublish(req, user, 'Menü mentés') }); }
        ...
        if (req.method === 'POST') {
          ...
          try {
            ...
            if (isVideoType(valid.type)) {
              ...
              keepStaging = true;
              dbSaved = true;
              await audit(req, { event_code: 'admin_media_uploaded', result: 'success', actor: user, scope_code: 'media', action_code: 'save', target_type: 'media', target_id: data.id, target_label: file.originalName, metadata: { mimeType: valid.type, size: file.size } });
            ...
            dbSaved = true;
            await audit(req, { event_code: 'admin_media_uploaded', result: 'success', actor: user, scope_code: 'media', action_code: 'save', target_type: 'media', target_id: data.id, target_label: file.originalName, metadata: { mimeType: stored.type, size: stored.size } });
            return json(res, 200, { ok: true, data });
          } catch (error) {
      ...
      if (url.pathname.startsWith('/api/admin/media/')) {
        ...
        if (req.method === 'PATCH' || req.method === 'PUT') {
          try { const payload = await body(req); const cls = classifyMediaMutation(media, payload); if (cls.noOp) return apiError(res, 400, 'INVALID_EMPTY_MUTATION', 'Nincs menthető változás.'); if ((cls.needsSave && !hasAction(adminContext.permissions, 'media', 'save')) || (cls.needsArchive && !hasAction(adminContext.permissions, 'media', 'archive'))) return apiError(res, 403, 'FORBIDDEN', 'Nincs jogosultság a művelethez.'); const data = await repo.updateMedia(id, payload); if (data) await audit(req, { event_code: media.status === 'archived' && data.status !== 'archived' ? 'admin_media_reactivated' : (data.status === 'archived' ? 'admin_media_archived' : 'admin_media_updated'), result: 'success', actor: user, scope_code: 'media', action_code: data.status === 'archived' ? 'archive' : 'save', target_type: 'media', target_id: id, target_label: media.path, metadata: { mimeType: media.type, altChanged: payload.alt !== undefined && payload.alt !== media.alt, previousStatus: media.status, nextStatus: data.status } }); return data ? json(res, 200, { ok: true, data }) : apiError(res, 404, 'MEDIA_NOT_FOUND', 'A média elem nem található.'); }
        ...
        if (req.method === 'DELETE') { try { const data = await repo.archiveMedia(id); if (data) await audit(req, { event_code: 'admin_media_archived', result: 'success', actor: user, scope_code: 'media', action_code: 'archive', target_type: 'media', target_id: id, target_label: media.path, metadata: { mimeType: media.type, previousStatus: media.status, nextStatus: 'archived' } }); return data ? json(res, 200, { ok: true, data }) : apiError(res, 404, 'MEDIA_NOT_FOUND', 'A média elem nem található.'); } catch (error) { if (error.status === 400 || error.code === 'VALIDATION_ERROR') return apiError(res, 400, 'INVALID_MEDIA', error.message); throw error; } }
      }
      if (url.pathname === '/api/admin/publish' && req.method === 'POST') return json(res, 200, { ok: true, publish: await auditedPublish(req, user, 'Kézi újraélesítés') });
      if (url.pathname.startsWith('/api/admin/publish/rollback/') && req.method === 'POST') { const snap = await repo.publishSnapshot(url.pathname.split('/').pop()); if (!snap) return apiError(res, 404, 'SNAPSHOT_NOT_FOUND', 'Snapshot nem található.'); const preflight = validateContentReferences(snap.content_json); if (!preflight.ok) return apiError(res, 409, 'CONTENT_REFERENCE_INVALID', referenceValidationSummary(preflight), preflight); const content = normalizeSnapshotForReferenceValidation(snap.content_json); await repo.importContentSnapshot(content); return json(res, 200, { ok: true, publish: await auditedPublish(req, user, `Rollback: ${snap.id}`, snap.id) }); }
      if (url.pathname === '/admin/dashboard') return redirect(res, '/admin/pages');
      if (url.pathname === '/admin/audit') return html(res, auditPanel(), 200, '/admin/audit', adminContext);
      ...
      console.error('admin server error', { code: error.code, message: error.message });
