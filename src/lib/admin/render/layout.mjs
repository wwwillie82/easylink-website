...
.card{background:#fff;border:1px solid #dfe6df;border-radius:18px;padding:20px;margin:14px 0;box-shadow:0 10px 30px #0f115912}
...
export const layout = (body, { nav = true, current = '' } = {}) => `<!doctype html><meta charset="utf-8"><title>Easylink site admin</title><style>${adminStyles}</style>${nav ? `<header class="admin-header"><div class="admin-bar"><h1 class="admin-title">Easylink site admin</h1><nav class="admin-nav" aria-label="Admin navigáció"><div class="admin-nav__links">${navItems.map(([href,label])=>`<a href="${href}"${current===href?' aria-current="page"':''}>${label}</a>`).join('')}</div><span class="admin-nav__spacer"></span><a class="admin-nav__logout" href="/api/admin/logout">Kilépés</a></nav></div></header>` : ''}<main class="wrap">${nav ? '' : '<h1>Easylink site admin</h1>'}${current === '/admin/media' ? '<p class="hint">A médiaválasztókban csak kész média választható.</p>' : ''}${body}</main><script>${videoDraftGuardJs()}</script>`;
