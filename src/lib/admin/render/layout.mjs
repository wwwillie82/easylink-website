...
function visibleNavItems(adminContext) { const permissions = adminContext?.permissions || {}; return adminNavItems.filter((item) => item.scope !== 'audit' && item.u1 && hasScope(permissions, item.scope)).map((item) => [item.href, item.label]); }
export const layout = (body, { nav = true, current = '', adminContext = null } = {}) => `<!doctype html><meta charset="utf-8"><title>Easylink site admin</title><style>${adminStyles}</style>${nav ? `<header class="admin-header"><div class="admin-bar"><h1 class="admin-title">Easylink site admin</h1><nav class="admin-nav" aria-label="Admin navigáció"><div class="admin-nav__links">${visibleNavItems(adminContext).map(([href,label])=>`<a href="${href}"${current===href?' aria-current="page"':''}>${label}</a>`).join('')}</div><span class="admin-nav__spacer"></span><form method="post" action="/api/admin/logout"><input type="hidden" name="_csrf" value=""><button class="admin-nav__logout" type="submit">Kilépés</button></form></nav></div></header>` : ''}<main class="wrap">${nav ? '' : '<h1>Easylink site admin</h1>'}${current === '/admin/media' ? '<p class="hint">Csak kész média választható a médiaválasztókban.</p>' : ''}${body}</main><script>${csrfFetchPatch()}${permissionGuardJs(current, adminContext)}${videoDraftGuardJs()}${ctaAdminEnhancementJs()}</script>`;
