...
import { adminNavItems, hasScope } from '../permissions.mjs';
const adminStyles = `
...
.admin-nav__account{display:grid;justify-items:center;gap:3px}.admin-nav__account form{margin:0}.admin-nav__user{display:grid;justify-items:center;font-size:.78rem;line-height:1.25;color:#59605f;white-space:nowrap}.admin-nav__user strong{color:#0f1159;font-size:.88rem}
...
@media(max-width:980px){.admin-grid--social{grid-template-columns:repeat(2,minmax(220px,1fr))}}@media(max-width:680px){.admin-grid,.grid,.admin-grid--compact,.admin-grid--social{grid-template-columns:1fr}.admin-section{margin:28px 0;padding:18px}.admin-subcard-header,.admin-save-bar{display:grid;align-items:stretch}.admin-toggle-row--lead{width:auto}.admin-save-bar{bottom:8px}.admin-form{padding-bottom:220px}.admin-toggle-row span{white-space:normal}.admin-page-header h2{font-size:1.55rem}.wrap,.admin-bar{padding:14px 18px}.admin-title{font-size:1.35rem;margin-bottom:8px}#msg{top:96px}.admin-nav{align-items:stretch}.admin-nav__links,.admin-nav{flex-direction:column}.admin-nav a{justify-content:center}.admin-nav__spacer{display:none}.admin-nav__account{width:100%;justify-items:stretch}.admin-nav__user{text-align:center;white-space:normal}.admin-nav__account button{width:100%}table{display:block;overflow-x:auto;white-space:nowrap}.item-row{grid-template-columns:1fr}.media-grid{grid-template-columns:1fr}.media-item img,.media-item video,.media-placeholder{height:180px}}
`;
function csrfFetchPatch() { return `(()=>{const read=(n)=>document.cookie.split(';').map(s=>s.trim()).find(s=>s.startsWith(n+'='))?.slice(n.length+1);const csrf=()=>decodeURIComponent(read('easylink_site_admin_csrf')||'');document.querySelectorAll('input[name=\"_csrf\"]').forEach(i=>i.value=csrf());const raw=window.fetch.bind(window);function sameAdmin(input){try{const u=new URL(typeof input==='string'?input:(input&&input.url)||'',window.location.href);return u.origin===window.location.origin&&u.pathname.startsWith('/api/admin/');}catch{return false;}}window.fetch=(input,init={})=>{const method=String(init.method||(input&&input.method)||'GET').toUpperCase();if(sameAdmin(input)&&['POST','PUT','PATCH','DELETE'].includes(method)){const h=new Headers(init.headers||(input&&input.headers)||{});if(!h.has('X-CSRF-Token'))h.set('X-CSRF-Token',csrf());init={...init,headers:h};}return raw(input,init);};})();`; }
function scopeForCurrent(current) { if (current.startsWith('/admin/pages')) return 'pages'; if (current === '/admin/menu') return 'menu'; if (current === '/admin/media') return 'media'; if (current === '/admin/settings') return 'settings'; if (current === '/admin/publish') return 'publish'; if (current === '/admin/users') return 'users'; return ''; }
function permissionGuardJs(current, adminContext) { const scope = scopeForCurrent(current || ''); const p = adminContext?.permissions?.[scope] || {}; return `(()=>{const p=${JSON.stringify(p)};const scope=${JSON.stringify(scope)};if(!scope)return;const disable=(el)=>{if(!el)return;el.disabled=true;el.setAttribute('aria-disabled','true');el.dataset.permissionDisabled='true';};if(!p.canSave){document.querySelectorAll('main form input,main form textarea,main form select,main form button[type=submit],[data-save-alt],[data-add-item],[data-add-related-link],[data-add-ai-item],[data-add-network-node],[data-add-network-edge]').forEach(disable);}if(!p.canArchive){document.querySelectorAll('[data-delete],[data-archive-media],[data-page-status],button.danger').forEach(disable);}if(!p.canDelete){document.querySelectorAll('[data-delete-page],[data-delete-navigation]').forEach(disable);}if(scope==='publish'){if(!p.canRepublish)document.querySelectorAll('[data-republish]').forEach(disable);if(!p.canRestore)document.querySelectorAll('[data-rollback]').forEach(disable);}})();`; }
function visibleNavItems(adminContext) { const permissions = adminContext?.permissions || {}; return adminNavItems.filter((item) => item.u1 && hasScope(permissions, item.scope)).map((item) => [item.href, item.label]); }
function currentUserLabel(adminContext) { return adminContext?.user?.displayName || adminContext?.user?.display_name || adminContext?.user?.email || ''; }
export const layout = (body, { nav = true, current = '', adminContext = null } = {}) => `<!doctype html><meta charset="utf-8"><title>Easylink site admin</title><style>${adminStyles}</style>${nav ? `<header class="admin-header"><div class="admin-bar"><h1 class="admin-title">Easylink site admin</h1><nav class="admin-nav" aria-label="Admin navigáció"><div class="admin-nav__links">${visibleNavItems(adminContext).map(([href,label])=>`<a href="${href}"${current===href?' aria-current="page"':''}>${label}</a>`).join('')}</div><span class="admin-nav__spacer"></span><div class="admin-nav__account">${currentUserLabel(adminContext) ? `<span class="admin-nav__user"><span>Bejelentkezve:</span><strong>${esc(currentUserLabel(adminContext))}</strong></span>` : ''}<form method="post" action="/api/admin/logout"><input type="hidden" name="_csrf" value=""><button class="admin-nav__logout" type="submit">Kilépés</button></form></div></nav></div></header>` : ''}<main class="wrap">${nav ? '' : '<h1>Easylink site admin</h1>'}${current === '/admin/media' ? '<p class="hint">Csak kész média választható a médiaválasztókban.</p>' : ''}${body}</main><script>${csrfFetchPatch()}${permissionGuardJs(current, adminContext)}${videoDraftGuardJs()}${ctaAdminEnhancementJs()}</script>`;
