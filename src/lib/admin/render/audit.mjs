...
<label>Esemény<select name="event_code"><option value="">Mind</option>${events}</select></label><label>Terület<input name="scope_code" placeholder="pages"></label>
<label>Felhasználó<input name="actor" placeholder="név vagy e-mail"></label><label>Cél típusa<input name="target_type"></label><label>Cél ID<input name="target_id"></label>
<label>Request ID<input name="request_id"></label><label>Keresés<input name="q" placeholder="esemény vagy cél"></label>
...
<div class="admin-table-scroll"><table id="audit-table"><thead><tr><th>Időpont</th><th>Felhasználó</th><th>Esemény</th><th>Terület</th><th>Művelet</th><th>Cél</th><th>Eredmény</th><th>Request ID</th><th>Részletek</th></tr></thead><tbody><tr><td colspan="9">Betöltés…</td></tr></tbody></table></div>
