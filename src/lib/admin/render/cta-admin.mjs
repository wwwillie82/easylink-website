export function ctaAdminEnhancementJs() {
  return String.raw`(()=>{
    ...
    let settingsEditor=null; let settingsDirty=false; let settingsSaving=false; let nativeFetch=globalThis.fetch?.bind(globalThis);
    ...
    function enhanceSettings(){
      ...
      const enforceSaveEnabled=()=>{if(settingsDirty&&!settingsSaving&&submit?.disabled)submit.disabled=false;};
      const submitStateObserver=submit&&globalThis.MutationObserver?new MutationObserver(enforceSaveEnabled):null;
      submitStateObserver?.observe(submit,{attributes:true,attributeFilter:['disabled']});
      const sync=()=>{
        mirrorSettingsLegacy(form);
        settingsDirty=true;
        enforceSaveEnabled();
        queueMicrotask(enforceSaveEnabled);
        setTimeout(enforceSaveEnabled,0);
      };
      ...
      form.addEventListener('submit',async(event)=>{
        ...
        try{const response=await nativeFetch('/api/admin/settings',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify(settingsPayload(form))});const json=await response.json();if(!json?.ok){settingsSaving=false;settingsMessage(json?.error?.message||'Mentési hiba.',false);if(submit)submit.disabled=false;return;}settingsDirty=false;settingsSaving=false;settingsMessage(json.publish?.ok?'Beállítások mentve és élesítve.':'Beállítások mentve, de az élesítés nem fejeződött be.',!!json.publish?.ok);setTimeout(()=>globalThis.location?.reload(),350);}
        catch{settingsSaving=false;settingsMessage('Hálózati hiba. Próbáld újra.',false);if(submit)submit.disabled=false;}
      },true);
    }
    if(nativeFetch){
