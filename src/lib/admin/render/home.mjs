...
import { homeClassificationNotice } from './home-classification.mjs';
import { allCanonicalHomeBlockKeys, HOME_HERO_META_KEY, HOME_INTRO_KEY, HOME_SOLUTIONS_KEY, HOME_AI_KEY, HOME_INTEGRATIONS_KEY, HOME_AUDIENCES_KEY, classifyHomeContentBlocks } from '../../content/home-blocks.mjs';
...
function technicalMeta(block) { const role = block.homeBlockRole || ''; return `<dl class="admin-tech-meta"><div><dt>ID</dt><dd>${esc(block.id ?? '')}</dd></div><div><dt>block_key</dt><dd>${esc(block.block_key || '')}</dd></div><div><dt>type</dt><dd>${esc(block.type || '')}</dd></div><div><dt>szerep</dt><dd>${esc(role)}</dd></div></dl>`; }
