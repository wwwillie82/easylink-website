...
export function navHtml(items, pages = [], options = {}) {
  const menuPerm = scopePermissions(options, 'menu');
  const canSave = menuPerm.canSave === true;
  const canArchive = menuPerm.canArchive === true;
  const canDelete = menuPerm.canDelete === true;
  const safePages = pages.map((p) => ({ id: Number(p.id), title: String(p.title || ''), route: String(p.route || ''), status: String(p.status || 'draft') }));
  return `<div class="admin-page admin-page--menu"><header class="admin-page-header"><h2>Menüpont-szerkesztő</h2><p class="admin-section-description">Belső oldalak, külső linkek és meglévő legacy URL-ek biztonságos kezelése.</p></header><div id="msg"></div><form class="admin-form admin-section" id="nav-form" data-can-save="${canSave ? '1' : '0'}" data-can-archive="${canArchive ? '1' : '0'}" data-can-delete="${canDelete ? '1' : '0'}"><header class="admin-section-header"><div><h3>Menüpontok</h3><p class="admin-section-description">Kártyánként válassz célmódot; archivált rekordok nem törlődnek.</p></div></header><div id="nav-rows" class="nav-card-list">${items.map((n, i) => navCard(n, safePages, i)).join('')}</div><div class="nav-list-actions">${canSave ? '<button type="button" id="add-nav">Menüpont hozzáadása</button>' : '<span class="hint">Nincs új menüpont létrehozási jog.</span><button type="button" id="add-nav" hidden disabled data-permission-disabled="true">Menüpont hozzáadása</button>'}</div><div class="admin-save-bar"><div><strong>Mentési terület</strong><p class="hint">A gomb módosítás után válik aktívvá; mentés automatikus élesítést indít.</p></div><button type="submit" ${canSave ? '' : 'disabled aria-disabled="true" data-permission-disabled="true"'}>Mentés és élesítés</button></div></form></div><script>${dirtyStateJs};${publishMessageJs}${menuAdminJs(safePages, { canSave, canArchive, canDelete })}</script>`;
}

function menuAdminJs(pages, caps = {}) { return `
...
const pages=${safeJson(pages)},menuCaps=${safeJson(caps)};window.menuCaps=menuCaps;const pageMap=new Map(pages.map(p=>[String(p.id),p]));
