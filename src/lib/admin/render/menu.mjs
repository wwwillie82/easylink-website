...
<header class="admin-subcard-header nav-card__header"><div><h4><span data-order>${esc(n.sort_order || index + 1)}</span>. ${esc(n.title || 'Új menüpont')}</h4><p class="hint">Azonosító: <code data-nav-id-label>${n.id ? esc(n.id) : 'új'}</code></p></div><div class="nav-card__badges"><span class="status-pill" data-status-badge>${esc(statusLabel(n.status || 'draft'))}</span><span class="status-pill" data-target-badge>${esc(targetLabel(type || ''))}</span></div></header>
...
${field('Cél típusa', `<select data-field="target_type" data-role="target-type"><option value="" ${!type ? 'selected' : ''}>Válassz célt</option><option value="page" ${type === 'page' ? 'selected' : ''}>Belső oldal</option><option value="external" ${type === 'external' ? 'selected' : ''}>Külső link</option>${!isNew ? `<option value="legacy" ${type === 'legacy' ? 'selected' : ''}>Régi kézi URL</option>` : ''}</select>`)}
${field('Sorrend', `<input data-field="sort_order" data-role="sort-visible" type="number" min="1" step="1" value="${esc(n.sort_order || index + 1)}">`)}
${field('Státusz', `<select data-field="status">${statusOptions(n.status || 'draft', { published: 'Látható', draft: 'Rejtett piszkozat', archived: 'Archivált' })}</select>`)}
</div>
<div data-mode="page">
...
<div class="admin-grid admin-grid--compact">${field('Örökölt / effektív felirat', `<input data-role="page-effective-title" readonly aria-readonly="true">`)}${field('Egyedi menüfelirat', `<input data-field="title_override" data-role="title-override" value="${esc(n.title_override || '')}">`)}${field('Aktuális oldalroute', `<input data-role="page-route" readonly aria-readonly="true">`)}${field('Oldal státusza', `<input data-role="page-status" readonly aria-readonly="true">`)}</div>
</div>
<div data-mode="external"><div class="admin-grid admin-grid--compact">${field('Kézi menüfelirat', `<input data-field="title" data-role="external-title" value="${esc(type === 'page' ? '' : n.title || '')}">`)}${field('Külső URL', `<input data-field="href" data-role="external-href" placeholder="https://…" value="${esc(type === 'page' ? '' : n.href || '')}">`)}</div></div>
<div data-mode="legacy"><div class="admin-grid admin-grid--compact">${field('Jelenlegi title', `<input data-role="legacy-title" value="${esc(n.title || '')}">`)}${field('Jelenlegi href', `<input data-role="legacy-href" value="${esc(n.href || '')}">`)}</div></div>
...
const state=setupDirtyForm(form,navSerializer);
...
function effectiveTitleHref(row,type=row.dataset.targetMode||row.querySelector('[data-role="target-type"]').value){if(type==='page'){const p=pageMap.get(row.querySelector('[data-role="page-select"]').value);return {title:p?pageTitle(row,p):(row.querySelector('[data-role="page-effective-title"]').value||''),href:p?.route||row.querySelector('[data-role="page-route"]').value||''};}if(type==='external')return {title:row.querySelector('[data-role="external-title"]').value,href:row.querySelector('[data-role="external-href"]').value};return {title:row.querySelector('[data-role="legacy-title"]').value,href:row.querySelector('[data-role="legacy-href"]').value};}
function updateRow(row){const type=row.querySelector('[data-role="target-type"]').value;row.dataset.targetMode=type;row.querySelector('[data-target-badge]').textContent=type?({page:'Belső oldal',external:'Külső link',legacy:'Régi kézi URL'}[type]):'Válassz célt';row.querySelector('[data-status-badge]').textContent={published:'Publikus',draft:'Piszkozat',archived:'Archivált'}[row.querySelector('[data-field="status"]').value]||'Piszkozat';row.querySelectorAll('[data-mode]').forEach(x=>x.hidden=x.dataset.mode!==type);const p=pageMap.get(row.querySelector('[data-role="page-select"]').value);const custom=row.querySelector('[data-role="title-mode"]').value==='custom';row.querySelector('[data-role="title-override"]').closest('label').hidden=!custom;row.querySelector('[data-role="page-effective-title"]').value=p?pageTitle(row,p):'';row.querySelector('[data-role="page-route"]').value=p?.route||'';row.querySelector('[data-role="page-status"]').value=p?({published:'Publikus',draft:'Piszkozat',archived:'Archivált'}[p.status]||p.status):(row.querySelector('[data-role="page-select"]').value?'Hiányzó oldal':'Nincs kiválasztva');row.classList.toggle('nav-card--archived',row.querySelector('[data-field="status"]').value==='archived');}
...
function applySavedIds(ids=[]){[...rows.querySelectorAll('[data-nav-item]')].forEach((row,i)=>{const id=ids[i];if(id){row.querySelector('[data-field="id"]').value=String(id);row.querySelector('[data-nav-id-label]').textContent=String(id);row.dataset.new='0';const select=row.querySelector('[data-role="target-type"]');if(![...select.options].some(o=>o.value==='legacy'))select.insertAdjacentHTML('beforeend','<option value="legacy">Régi kézi URL</option>');}});}
...
document.getElementById('add-nav').onclick=()=>{rows.insertAdjacentHTML('beforeend',newCardHtml);renumber();[...rows.querySelectorAll('[data-nav-item]')].forEach(updateRow);};
[...rows.querySelectorAll('[data-nav-item]')].forEach(updateRow);
form.onsubmit=async e=>{e.preventDefault();if(!state.changed())return;let items;try{items=serializeAll();}catch(error){menuMsg(error.message,false);return;}state.markSaving();const r=await fetch('/api/admin/navigation',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({items})});const j=await r.json();if(j.ok){applySavedIds(j.data?.navigationIds||[]);state.markSaved();menuMsg(pm(j.publish),j.publish?.ok);return;}menuMsg(j.error?.message||'Mentési hiba',false);};`; }
