...
export function serializeVideoEditorValues(values = {}) {
  ...
  const cfg = {
    ...
    controls: values.autoplay === false ? true : values.controls !== false,
...
export function syncVideoUiState(form, setVisibleFn = setVideoEditorVisible) {
  ...
  const mutedNote = form?.querySelector?.('[data-video-muted-note]');
  const controlsNote = form?.querySelector?.('[data-video-controls-note]');
  const youtubeNote = form?.querySelector?.('[data-video-youtube-controls-note]');
  if (!autoplay && controls) controls.checked = true;
  if (mutedNote) mutedNote.hidden = !(autoplay && muted && !muted.checked);
  if (controlsNote) controlsNote.hidden = autoplay;
  if (youtubeNote) youtubeNote.hidden = visibility.sourceType !== 'youtube';
  return visibility;
}
