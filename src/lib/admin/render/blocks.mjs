...
const supportedBlockTypes = new Set(['text','feature-list','list','cards','card-grid','cta','image-text','faq','ai-preview','network-visual']);
...
function imagePanel(first, selectedKind) { if (selectedKind !== 'image-text') return ''; return `<div class="grid" data-panel="image-text"><label>Kép URL<input data-image-url value="${esc(first.image || '')}"></label><button type="button" class="secondary" data-media-picker-target="[data-image-url]">Médiából választok</button><label>Alt text<input data-image-alt value="${esc(first.alt || '')}"></label><label>Kép pozíció<select data-image-position><option value="right" ${first.position!=='left'?'selected':''}>jobb</option><option value="left" ${first.position==='left'?'selected':''}>bal</option></select></label></div>`; }
