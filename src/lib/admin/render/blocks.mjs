...
function aiPreviewPanel(items, selectedKind) { if (selectedKind !== 'ai-preview') return ''; return `<div data-panel="ai-preview" data-ai-preview-editor><p class="hint">AI előnézet elemek strukturált szerkesztése. A JSON export csak haladó/fallback célra marad.</p>${aiPreviewRows(items)}<button type="button" data-add-ai-item>Új AI elem</button></div><details data-panel="ai-preview-raw"><summary>Haladó JSON export / fallback</summary><textarea data-ai-preview-json-export readonly>${esc(JSON.stringify(items, null, 2))}</textarea></details>`; }
