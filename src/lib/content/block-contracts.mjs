...
import { hasTooManyCtaButtons, incompleteCtaButton, itemCtaButtons, withLegacyItemFields } from './cta-buttons.mjs';
...
function normalizeCardInput(item = {}, path = 'items', errors = {}) {
  ...
  try { target_type = rawCardTargetType(target_type); }
  catch { field(errors, `${path}.target_type`, 'Hibás cél típus.'); target_type = 'legacy'; }
  ...
  const out = {
    ...
    text: firstNonEmpty(textOverride, source.text, source.shortDescription, source.body),
    text_override: textOverride,
    linkLabel: clean(source.linkLabel ?? source.label ?? ''),
    badge: source.badge ?? source.order ?? '',
  };
  if (target_type === 'page') {
    const id = Number(source.target_page_id);
    if (!Number.isSafeInteger(id) || id <= 0) field(errors, `${path}.target_page_id`, 'Válassz publikus céloldalt.');
    else out.target_page_id = id;
  } else if (target_type === 'legacy') {
    const href = clean(source.href ?? source.url);
    const slug = clean(source.slug);
    if (href && !isInternalRouteCandidate(href)) field(errors, `${path}.href`, 'Legacy cél csak biztonságos belső útvonal lehet.');
    out.href = href;
    if (slug) out.slug = slug;
  } else if (target_type === 'external') {
    const href = clean(source.href ?? source.url);
    if (href && !isValidHttpExternalUrl(href)) field(errors, `${path}.href`, 'Külső cél csak http(s) URL lehet.');
    out.href = href;
  }
  return out;
}
...
export function publicCardsFromItems(items = [], { pages = [] } = {}) {
  ...
  const resolve = (entry, index, isAction = false) => {
    ...
    try {
      ...
      if (entry.target_type === 'page') throw error;
      return isAction ? { href: entry.href || '', label: entry.label || entry.title || '' } : { ...entry, href: entry.href || entry.url || '', url: entry.href || entry.url || '' };
    }
  };
...
export function normalizeAiAssistantItems(items = [], { fieldErrors = {}, path = 'items', status = 'published' } = {}) {
  ...
  const messages = items.filter((item) => clean(item?.kind) === 'message').map((item, index) => {
    const role = roles.has(clean(item.role)) ? clean(item.role) : 'assistant';
    if (clean(item.role) && !roles.has(clean(item.role))) field(fieldErrors, `${path}.${index}.role`, 'Ismeretlen üzenet szerep.');
    return { kind: 'message', role, title: clean(item.title), text: String(item.text ?? '').trim(), order: item.order ?? index + 1 };
  }).filter((item) => item.text || item.title);
  if (status === 'published') {
    if (!heading) field(fieldErrors, `${path}.0.heading`, 'Egy heading kötelező.');
    if (!sources.length) field(fieldErrors, `${path}.sources`, 'Legalább egy source kötelező.');
    if (!messages.some((message) => message.role === 'user' && message.text)) field(fieldErrors, `${path}.messages`, 'Legalább egy user üzenet kötelező.');
    if (!messages.some((message) => message.role === 'assistant' && message.text)) field(fieldErrors, `${path}.messages`, 'Legalább egy assistant üzenet kötelező.');
  }
...
export function normalizeIntegrationsStripItems(items = [], { fieldErrors = {}, path = 'items', status = 'published' } = {}) {
  ...
  for (const [index, item] of items.entries()) {
    ...
    const label = clean(item.label || item.title);
    ...
    seen.add(id);
    if (!label) field(fieldErrors, `${path}.${index}.label`, 'A label kötelező.');
    if (id && label) nodes.push({ kind: 'node', id, label, order: item.order ?? nodes.length + 1 });
  }
  if (status === 'published') {
    if (!heading) field(fieldErrors, `${path}.0.heading`, 'Egy heading kötelező.');
    if (!nodes.length) field(fieldErrors, `${path}.nodes`, 'Legalább egy integration item kötelező.');
  }
  return [{ kind: 'heading', text: heading }, ...nodes];
}

export function normalizeCtaItems(items = [], { fieldErrors = {}, path = 'items' } = {}) {
  if (!items.length) return [];
  const source = obj(items[0]) ? { ...items[0] } : {};
  if (hasTooManyCtaButtons(source)) field(fieldErrors, `${path}.0.buttons`, 'Legfeljebb 4 CTA gomb adható meg.');
  const buttons = itemCtaButtons(source);
  const safeUrl = (value) => !clean(value) || isInternalRouteCandidate(clean(value)) || isValidHttpExternalUrl(clean(value)) || clean(value).startsWith('mailto:') || clean(value).startsWith('tel:');
  buttons.forEach((button, index) => {
    if (incompleteCtaButton(button)) field(fieldErrors, `${path}.0.buttons.${index}`, 'A CTA gomb felirata és célja együtt kötelező.');
    if (!safeUrl(button.url)) field(fieldErrors, `${path}.0.buttons.${index}.url`, 'A CTA link csak biztonságos belső, http(s), mailto vagy tel URL lehet.');
  });
  ...
  return [withLegacyItemFields({ ...source, headerHidden: source.headerHidden === true }, buttons)];
}

export function normalizeBlockItemsByType(type, items, options = {}) {
  if (items === null || items === undefined) items = [];
  if (!Array.isArray(items)) {
    const error = new Error('Az items mezőnek JSON tömbnek kell lennie.');
    error.code = 'INVALID_BLOCK_ITEMS';
    throw error;
  }
  ...
  if (type === 'cta') return normalizeCtaItems(items, options);
